...
std::string PluginHTTP::PerformHTTPWithRequestID(std::string receivedData, std::string requestID)
{
    ...
    {
        g_SMAPI->ConPrintf("[PerformHTTP] An error has occured while trying to create the HTTP Request: %s\n", GetParseError_En(request.GetParseError()));
    ...
    if (methodMap.find(method) == methodMap.end()) {
        g_SMAPI->ConPrintf("[PerformHTTP] An error has occured while trying to create the HTTP Request: Invalid method \"%s\"\n", method.c_str());
        return "00000000-0000-0000-0000-000000000000";
    }
    ...
    if (methodMap.at(method) != k_EHTTPMethodGET && methodMap.at(method) != k_EHTTPMethodDELETE) {
        if (!g_http->SetHTTPRequestRawPostBody(req, contentType.c_str(), (uint8*)(data.c_str()), data.size())) {
            g_SMAPI->ConPrint("[PerformHTTP] An error has occured while trying to create the HTTP Request: Couldn't copy the body data. Possibly invalid content type.\n");
            return "00000000-0000-0000-0000-000000000000";
        }
    }
